    ...
    <article className="group flex flex-col overflow-hidden rounded-sm border border-[#E8DFC8] bg-white transition-shadow duration-300 hover:shadow-md">
      ...
      <div className="relative aspect-[3/4] overflow-hidden bg-[#F5EFE0]">
        ...
        <div className="absolute left-3 top-3 flex flex-col gap-1.5">
          {badge && (
            <Badge
              className={`text-[10px] font-medium tracking-wide ${
                BADGE_COLORS[badge] ?? "bg-[#C9A84C] text-[#2C2416]"
          ...
          {!inStock && (
            <Badge className="bg-[#2C2416]/60 text-[#FAF7F0] text-[10px] border-0 hover:opacity-90">
              Out of Stock
            </Badge>
          )}
        </div>
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        <div className="mb-1.5 flex flex-wrap items-center gap-1.5">
          ...
          <span className="font-body text-[10px] text-[#7A6A52]">{fabric}</span>
        </div>
